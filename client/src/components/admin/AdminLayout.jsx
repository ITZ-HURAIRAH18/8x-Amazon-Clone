...
export default function AdminLayout() {
  ...
  return <div className={`admin-app ${collapsed ? "admin-app--collapsed" : ""}`}>
    ...
    <div className="admin-body"><aside ref={sidebarRef} className={`admin-sidebar ${mobileOpen ? "admin-sidebar--open" : ""}`}><div className="admin-sidebar__mobile-head"><strong>Admin navigation</strong><button type="button" className="admin-icon-button" onClick={() => setMobileOpen(false)} aria-label="Close admin navigation"><X size={19} /></button></div><nav aria-label="Admin navigation">{links.map(({ to, label, icon: Icon, badge }) => <NavLink key={to} to={to} className={({ isActive }) => isActive ? "is-active" : ""} onClick={() => setMobileOpen(false)}><Icon size={18} /><span>{label}</span>{badge && counts[badge] > 0 && <b>{counts[badge]}</b>}</NavLink>)}</nav><div className="admin-sidebar__footer"><Link to="/" target="_blank"><Home size={16} /> View storefront</Link><button type="button" onClick={() => setCollapsed((value) => !value)}>{collapsed ? <ChevronRight size={16} /> : <ChevronLeft size={16} />} {collapsed ? "Expand" : "Collapse"}</button></div></aside>{mobileOpen && <button className="admin-sidebar-overlay" type="button" onClick={() => setMobileOpen(false)} aria-label="Close navigation" />}</div>
