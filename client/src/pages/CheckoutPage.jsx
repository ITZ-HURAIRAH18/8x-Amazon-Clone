...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    let active = true
    addressApi.list().then((items) => { if (!active) return; setAddresses(items || []); const preferred = items?.find((item) => item.isDefault) || items?.[0]; if (preferred) setSelectedAddressId(preferred.id) }).catch((requestError) => { if (active) setError(errorMessage(requestError, "Saved addresses could not be loaded.")) })
    return () => { active = false }
  }, [])
  ...
  const applyCoupon = async (event) => {
    ...
    try { const result = await couponApi.validate(couponCode, subtotal); setCoupon({ code: result.coupon.code, discount: result.discount, shippingCoupon: result.coupon.discountType === "shipping" }) } catch (requestError) { setCoupon(null); setCouponError(errorMessage(requestError, "This coupon could not be applied.")) } finally { setCouponBusy(false) }
  }
  const placeOrder = async (event) => {
    event.preventDefault(); setBusy(true); setError("")
    const shippingAddress = selectedAddress || newAddress
    try {
      const order = await orderApi.create({ addressId: selectedAddress?.id, shippingAddress, deliveryMethod, paymentMethod: payment, couponCode: coupon?.code || "", clientRequestId: `${user?._id || user?.id || "user"}-${Date.now()}` })
