...
export default function CartPage() {
  ...
  const tax = Math.max(0, subtotal - (coupon?.discount || 0)) * 0.08
  ...
  const applyCoupon = async (event) => {
    ...
    try {
      if (user) {
        ...
        setCoupon({ code, discount: result.discount, shippingCoupon: code === "FREESHIP", shipping: result.shipping })
