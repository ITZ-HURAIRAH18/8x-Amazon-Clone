...
export default function ProductDetailPage() {
  ...
  const specifications = product?.specifications || { Brand: product?.brand, Category: product?.category, Shipping: "Ships from Amazon Clone", Returns: "30-day returns" }
  ...
  return <div className="product-detail-page"><div className="container">
    ...
    <div className="detail-lower"><section className="feature-panel"><h2>Product details</h2><ul>{(product.features || []).map((feature) => <li key={feature}><Check size={16} />{feature}</li>)}</ul>{specifications && <table className="spec-table"><tbody>{Object.entries(specifications).map(([key, value]) => <tr key={key}><th>{key}</th><td>{String(value)}</td></tr>)}</tbody></table>}<p>{product.description}</p></section><section className="review-panel" id="reviews"><ReviewSection reviews={reviews} user={user} productId={product.id} onSubmit={submitReview} onChanged={refreshReviews} busy={reviewBusy} message={reviewMessage} /></section></div>
