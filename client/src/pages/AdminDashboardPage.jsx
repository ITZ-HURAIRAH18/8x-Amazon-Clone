...
export default function AdminDashboardPage() {
  ...
  // The dashboard endpoint returns MongoDB aggregation output under `sales`.
  const sales = data?.sales || {}
  const revenueOverTime = data?.revenueOverTime || (sales.timeline || []).map((point) => ({ ...point, label: point.date, value: point.revenue }))
  const ordersOverTime = data?.ordersOverTime || (sales.timeline || []).map((point) => ({ ...point, label: point.date, value: point.orders }))
  const salesByCategory = data?.salesByCategory || (sales.byCategory || []).map((row) => ({ ...row, label: row.name, value: row.revenue }))
  const salesByBrand = data?.salesByBrand || (sales.byBrand || []).map((row) => ({ ...row, label: row.name, value: row.revenue }))
  const rawStatusDistribution = data?.statusDistribution || orders.statusDistribution || {}
  const statusDistribution = Array.isArray(rawStatusDistribution)
    ? rawStatusDistribution
    : Object.entries(rawStatusDistribution).map(([label, value]) => ({ label, value, count: value }))
  const recentOrders = data?.recentOrders || []
  const lowStock = data?.lowStock || data?.lowStockProducts || []
