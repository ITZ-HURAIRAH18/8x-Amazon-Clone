...
export default function AdminSearchPage() {
  ...
  useEffect(() => { const query = (params.get("q") || "").trim(); if (query.length < 2) { setResult(null); setLoading(false); setError(""); return } setLoading(true); adminApi.search(query).then((response) => setResult(payloadOf(response))).catch((requestError) => setError(errorMessage(requestError, "Admin search failed."))).finally(() => setLoading(false)) }, [params])
  return <div className="admin-page"><AdminPageHeader eyebrow="Admin search" title="Search operations" description="Find products, orders, customers, promotions, and taxonomy records from one place." /><form className="admin-global-search-form" onSubmit={(event) => { event.preventDefault(); search(term.trim()) }}><Search size={19} /><input value={term} onChange={(event) => setTerm(event.target.value)} placeholder="Search products, orders, customers…" autoFocus /><button className="admin-primary-button" type="submit">Search</button></form>{error && <AdminError message={error} />}{loading ? <AdminLoading label="Searching admin data…" /> : !params.get("q") || params.get("q").trim().length < 2 ? <AdminEmpty title="Search the operations database" message="Enter at least two characters: a product name, order number, customer email, or coupon code." /> : !result || groups.every((group) => !(result[group.key]?.length)) ? <AdminEmpty title={`No results for “${params.get("q")}”`} message="Try a shorter or more general search." /> : <div className="admin-search-results">{groups.map(({ key, label, icon: Icon, path }) => { const rows = result[key] || []; if (!rows.length) return null; return <section className="admin-panel" key={key}><div className="admin-panel__heading"><div><Icon size={19} /><h2>{label} <small>({result.results?.[key]?.total ?? rows.length})</small></h2></div><Link to={path}>Manage <span>→</span></Link></div><div className="admin-search-result-list">{rows.map((row) => <Link to={row.link || path} key={row.id || row._id || row.code}><strong>{row.title || row.name || row.orderNumber || row.email || row.code}</strong><span>{row.subtitle || row.status || row.sku || ""}</span></Link>)}</div></section> })}</div>}</div>
}
